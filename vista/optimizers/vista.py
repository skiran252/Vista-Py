...
class VistaOptimizer:
    ...
    async def optimize(
        ...
        for epoch in range(1, epochs + 1):
            ...
            if epoch_best_score > best_score:
                ...
                print(
                    f"Iteration {epoch} failed to improve (Best candidate scored {epoch_best_score:.2f} <= {best_score:.2f})"
                )
    ...
    async def _do_random_restart(self, current_prompt: str) -> str:
        ...
        if self.llm_client.api_base:
            completion_kwargs["api_base"] = self.llm_client.api_base

        res = await litellm.acompletion(**completion_kwargs)
