...
class LLMClient:
    ...
    async def predict(self, signature: Signature, inputs: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            # Fallback, just return the raw text if parsing fails (shouldn't happen with structured outputs)
            return {signature.outputs[0].name: raw_output}

    async def generate_structured(self, messages: list, response_model: type[BaseModel]) -> BaseModel:
